...
export async function GET_AVAILABILITY(request: NextRequest) {
  try {
    ...
    if (!agentId || !date) {
      return NextResponse.json(
        { success: false, error: 'Agent ID and date are required' },
        { status: 400 }
      );
    }

    // Get all showings for this agent on this date
    ...
    for (let hour = 9; hour < 18; hour++) {
      for (let minute = 0; minute < 60; minute += 30) {
        ...
        const isBooked = showings?.some(showing => {
          const showingTime = showing.scheduled_time;
          return showingTime === time;
        });
